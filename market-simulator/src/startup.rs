...
pub fn start_execution_report_engine(
    ...
    let mut execution_report_engine =
        ExecutionReportEngine::new(er_rx, er_tx, Arc::clone(&shutdown));
    ...
    let _er_thread = std::thread::spawn(move || {
        ...
        match execution_report_engine.run() {
            ...
            Err(e) => {
                tracing::error!(
                    "[{}] Execution report engine error: {e:#}",
                    utils::market_name()
                );
...
pub fn start_db_engine(
    ...
    // DB engine thread
    let db_engine =
        match db::DatabaseEngine::new(ob_db_rx, &database_url, Arc::clone(&global_shutdown)) {
            Ok(engine) => engine,
            Err(e) => {
...
pub fn start_grpc_server(
    ...
    let _grpc_thread = std::thread::spawn(move || {
            ...
            .build()
        {
            ...
            Err(e) => {
                tracing::error!(
                    "[{}] Failed to build tokio runtime for gRPC server: {e:#}",
                    utils::market_name()
                );
