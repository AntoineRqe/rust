...
pub async fn metrics_handler(
    ...
    let login_samples = metrics
        .login_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    let (login_buckets, login_sum, login_count) =
        calculate_histogram_stats(&login_samples, LATENCY_BUCKETS_MS);

    for (bucket_bound, count) in login_buckets {
        buffer.push_str(&format!(
            "login_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "login_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        login_count
    ));
    ...
    let order_samples = metrics
        .order_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    let (order_buckets, order_sum, order_count) =
        calculate_histogram_stats(&order_samples, LATENCY_BUCKETS_MS);

    for (bucket_bound, count) in order_buckets {
        buffer.push_str(&format!(
            "order_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "order_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        order_count
    ));
    ...
    let exec_samples = metrics
        .execution_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    let (exec_buckets, exec_sum, exec_count) =
        calculate_histogram_stats(&exec_samples, LATENCY_BUCKETS_MS);

    for (bucket_bound, count) in exec_buckets {
        buffer.push_str(&format!(
            "execution_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "execution_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        exec_count
    ));
    ...
    let ob_samples = metrics
        .order_book_event_to_fanout_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in ob_buckets {
        buffer.push_str(&format!(
            "order_book_event_to_fanout_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "order_book_event_to_fanout_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        ob_count
    ));
    buffer.push_str(&format!(
        "order_book_event_to_fanout_latency_ms_sum {}\n",
        ob_sum
    ));
    buffer.push_str(&format!(
        "order_book_event_to_fanout_latency_ms_count {}\n",
        ob_count
    ));

    buffer
        .push_str("# HELP execution_report_events_total Total execution report events processed\n");
    buffer.push_str("# TYPE execution_report_events_total counter\n");
    buffer.push_str(&format!(
        "execution_report_events_total {}\n",
        metrics
    ...
    let er_samples = metrics
        .execution_report_event_to_fanout_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in er_buckets {
        buffer.push_str(&format!(
            "execution_report_event_to_fanout_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "execution_report_event_to_fanout_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        er_count
    ));
    buffer.push_str(&format!(
        "execution_report_event_to_fanout_latency_ms_sum {}\n",
        er_sum
    ));
    buffer.push_str(&format!(
        "execution_report_event_to_fanout_latency_ms_count {}\n",
        er_count
    ));
    ...
    let db_samples = metrics
        .order_db_write_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in db_buckets {
        buffer.push_str(&format!(
            "order_db_write_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "order_db_write_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        db_count
    ));
    ...
    let fix_samples = metrics
        .fix_request_to_response_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    let (fix_buckets, fix_sum, fix_count) =
        calculate_histogram_stats(&fix_samples, LATENCY_BUCKETS_MS);

    for (bucket_bound, count) in fix_buckets {
        buffer.push_str(&format!(
            "fix_request_to_response_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "fix_request_to_response_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        fix_count
    ));
    buffer.push_str(&format!(
        "fix_request_to_response_latency_ms_sum {}\n",
        fix_sum
    ));
    buffer.push_str(&format!(
        "fix_request_to_response_latency_ms_count {}\n",
        fix_count
    ));
    ...
    buffer.push_str(
        "# HELP player_api_errors_total Total failed backend to player service API calls\n",
    );
    buffer.push_str("# TYPE player_api_errors_total counter\n");
    buffer.push_str(&format!(
        "player_api_errors_total {}\n",
        metrics
            .player_api_errors
            .load(std::sync::atomic::Ordering::Relaxed)
    ));
    ...
    buffer.push_str(
        "# HELP player_api_latency_ms Backend to player service API call latency in milliseconds\n",
    );
    buffer.push_str("# TYPE player_api_latency_ms histogram\n");

    let player_samples = metrics
        .player_api_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    let (player_buckets, player_sum, player_count) =
        calculate_histogram_stats(&player_samples, LATENCY_BUCKETS_MS);

    for (bucket_bound, count) in player_buckets {
        buffer.push_str(&format!(
            "player_api_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "player_api_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        player_count
    ));
    ...
    let ui_samples = metrics
        .ui_order_round_trip_latency_ms
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in ui_buckets {
        buffer.push_str(&format!(
            "ui_order_round_trip_latency_ms_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "ui_order_round_trip_latency_ms_bucket{{le=\"+Inf\"}} {}\n",
        ui_count
    ));
    buffer.push_str(&format!("ui_order_round_trip_latency_ms_sum {}\n", ui_sum));
    buffer.push_str(&format!(
        "ui_order_round_trip_latency_ms_count {}\n",
        ui_count
    ));
    ...
    let ws_samples = metrics
        .websocket_fanout_to_browser_latency_us
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in ws_buckets {
        buffer.push_str(&format!(
            "websocket_fanout_to_browser_latency_us_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "websocket_fanout_to_browser_latency_us_bucket{{le=\"+Inf\"}} {}\n",
        ws_count
    ));
    buffer.push_str(&format!(
        "websocket_fanout_to_browser_latency_us_sum {}\n",
        ws_sum
    ));
    buffer.push_str(&format!(
        "websocket_fanout_to_browser_latency_us_count {}\n",
        ws_count
    ));
    ...
    let ws_send_samples = metrics
        .websocket_send_latency_us
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in ws_send_buckets {
        buffer.push_str(&format!(
            "websocket_send_latency_us_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "websocket_send_latency_us_bucket{{le=\"+Inf\"}} {}\n",
        ws_send_count
    ));
    buffer.push_str(&format!("websocket_send_latency_us_sum {}\n", ws_send_sum));
    buffer.push_str(&format!(
        "websocket_send_latency_us_count {}\n",
        ws_send_count
    ));

    buffer.push_str(
        "# HELP websocket_player_state_send_latency_us Player state send latency in microseconds\n",
    );
    buffer.push_str("# TYPE websocket_player_state_send_latency_us histogram\n");

    let ps_samples = metrics
        .websocket_player_state_send_latency_us
        .lock()
        .map(|s| s.clone())
        .unwrap_or_default();
    ...
    for (bucket_bound, count) in ps_buckets {
        buffer.push_str(&format!(
            "websocket_player_state_send_latency_us_bucket{{le=\"{}\"}} {}\n",
            bucket_bound, count
        ));
    }
    buffer.push_str(&format!(
        "websocket_player_state_send_latency_us_bucket{{le=\"+Inf\"}} {}\n",
        ps_count
    ));
    buffer.push_str(&format!(
        "websocket_player_state_send_latency_us_sum {}\n",
        ps_sum
    ));
    buffer.push_str(&format!(
        "websocket_player_state_send_latency_us_count {}\n",
        ps_count
    ));
