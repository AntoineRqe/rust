...
pub fn parse_tag_fast(bytes: &[u8]) -> u32 {
    ...
    match bytes.len() {
        ...
        2 => (bytes[0] - b'0') as u32 * 10 + (bytes[1] - b'0') as u32,
        3 => {
            (bytes[0] - b'0') as u32 * 100
                + (bytes[1] - b'0') as u32 * 10
                + (bytes[2] - b'0') as u32
        }
        4 => {
            (bytes[0] - b'0') as u32 * 1000
...
impl<'a> FixParser<'a> {
    ...
    pub fn get_fields(&mut self) -> FixMessage<'a> {
        ...
        while i + 64 <= len {
            ...
            while soh_bits != 0 {
                ...
                let eq_pos_abs = if range_mask != 0 {
                    ...
                    match data[start_field..field_end]
                        .iter()
                        .position(|&b| b == EQUALS)
                    {
        ...
        while i < len {
            if data[i] == SOH {
                let field_end = i;
                if let Some(eq_pos) = data[start_field..field_end]
                    .iter()
                    .position(|&b| b == EQUALS)
                {
