...
pub async fn start_server(
    ...
    let app = Router::new()
        .route(
            ...
            get(move |ws: WebSocketUpgrade| {
                ...
                async move {
                    tracing::info!(
                        "[{}] New WebSocket connection for market feed",
                        utils::market_name()
                    );
        ...
        .route(
            ...
            get(move |ws: WebSocketUpgrade| {
                ...
                async move {
                    tracing::info!(
                        "[{}] New WebSocket connection for snapshot feed",
                        utils::market_name()
                    );
                    // Upgrade to WebSocket and handle the connection, on_upgrade spawns a new task for each connection
                    ws.on_upgrade(move |socket| handle_ws(socket, tx))
    ...
    let shutdown_signal = async move {
        ...
        tracing::info!(
            "[{}] Axum server shutdown signal received",
            utils::market_name()
        );
    };
    ...
    tracing::info!(
        "[{}] WebSocket server on {} has shut down",
        utils::market_name(),
        addr
    );
...
async fn handle_ws(mut socket: WebSocket, tx: broadcast::Sender<Vec<u8>>) {
    ...
    while let Ok(msg) = rx.recv().await {
        ...
        tracing::debug!(
            "[{}] Sending {} bytes to WebSocket client",
            utils::market_name(),
            bytes.len()
        );
