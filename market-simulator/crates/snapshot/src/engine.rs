    ...
    pub fn new(
        ...
        Self {
            ...
            source: SourceSocket::new(ip, port, market_name())
                .expect("Failed to create multicast socket for SnapshotMultiCastEngine"),
        }
    }
        ...
        while !self.shutdown.load(std::sync::atomic::Ordering::Relaxed) || !self.fifo_in.is_empty()
        {
            if let Some(snapshot) = self.fifo_in.pop() {
                if snapshot.timestamp_ms == 0 {
                    tracing::info!(
                        "[{}] Received shutdown signal, stopping SnapshotMultiCastEngine",
                        market_name()
                    );
                    continue;
                }
                ...
                {
                    tracing::error!(
                        "[{}] Failed to send snapshot over multicast: {e:#}",
                        market_name()
                    );
                }
            }
