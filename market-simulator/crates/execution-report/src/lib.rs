...
pub struct ExecutionReportEngine<'a, const N: usize> {
    ...
    fifo_out: Producer<'a, FixRawMsg<N>, N>,
    running: AtomicBool,
}

impl<'a, const N: usize> ExecutionReportEngine<'a, N> {
    pub fn new(fifo_in: Consumer<'a, (OrderEvent, OrderResult), N>, fifo_out: Producer<'a, FixRawMsg<N>, N>) -> Self {
    ...
    fn build_execution_report(&self, exec_report: &(OrderEvent, OrderResult)) -> FixRawMsg<N> {
        ...
        match order.side {
            order_book::types::Side::Buy => self.build_field(tags::SIDE, side_code_set::BUY, &mut report, &mut cursor),
            order_book::types::Side::Sell => self.build_field(tags::SIDE, side_code_set::SELL, &mut report, &mut cursor),
        }
        ...
        if !order_result.trades.is_empty() {
            ...
            self.build_field(tags::AVG_PX, &number_to_bytes(0u64).as_ref(), &mut report, &mut cursor);
        }
    ...
    fn process_execution_report(&self, exec_report: &(OrderEvent, OrderResult)) {
        ...
        match self.fifo_out.push(raw_report) {
...
mod tests {
    ...
    fn test_execution_report_engine() {
        ...
        let mut rb_out = spsc::spsc_lock_free::RingBuffer::<FixRawMsg<1024>, 1024>::new();
        ...
        std::thread::scope(|s| {
            ...
            let order_event = order_book::types::OrderEvent {
                order_type: order_book::types::OrderType::LimitOrder,
                cl_ord_id: order_book::types::OrderId::from_ascii("CLORD12345"),
                order_id: order_book::types::OrderId::from_ascii("ORDERID"),
                side: order_book::types::Side::Buy,
                price: order_book::types::Price(123_456_000), // 123.456 in FIX price format (8 decimal places)
                quantity: 100,
                sender_id: order_book::types::EntityId::from_ascii("SENDER"),
                target_id: order_book::types::EntityId::from_ascii("TARGET"),
                symbol: order_book::types::FixedString::from_ascii("TEST_SYMBOL"),
                timestamp: 0,
            };

            let order_result = order_book::types::OrderResult {
                trades: vec![],
                status: order_book::types::OrderStatus::New,
            };
            ...
            let raw_report = fifo_out_rx.pop().expect("No execution report generated");
            ...
            let sell_order_event = order_book::types::OrderEvent {
                order_type: order_book::types::OrderType::LimitOrder,
                cl_ord_id: order_book::types::OrderId::from_ascii("CLORD54321"),
                order_id: order_book::types::OrderId::from_ascii("ORDERID2"),
                side: order_book::types::Side::Sell,
                price: order_book::types::Price(123_456_000), // 123.456 in FIX price format (8 decimal places)
                quantity: 50,
                sender_id: order_book::types::EntityId::from_ascii("SENDER2"),
                target_id: order_book::types::EntityId::from_ascii("TARGET2"),
                symbol: order_book::types::FixedString::from_ascii("TEST_SYMBOL"),
                timestamp: 0,
            };

            let sell_order_result = order_book::types::OrderResult {
                trades: vec![order_book::types::Trade { id: order_book::types::TradeId::default(), quantity: 50, price: order_book::types::Price(123_456_000) }],
                status: order_book::types::OrderStatus::PartiallyFilled,
            };
            ...
            std::thread::sleep(std::time::Duration::from_millis(100)); // Give the engine some time to process
            let raw_report = fifo_out_rx.pop().expect("No execution report generated for sell order");
            let mut fix_parser = fix::parser::FixParser::new(&raw_report.data[..raw_report.len as usize]);
            let parsed_report = fix_parser.get_fields();    
